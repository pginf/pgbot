...
class GroupMSG(commands.Cog):
    def __init__(self, bot: PGBot):
        self.bot: PGBot = bot

    @cog_ext.cog_slash(name="groupmsg", description="Wyświetla informacje o użytkowniku", guild_ids=guild_ids)
    async def __groupmsg(self, ctx: SlashContext):

        if ctx.author_id != 354322592787857408:
            return
        ...
        await ctx.send("Aby dołączyć do konkretnej grupy dziekańskiej zaznacz prawidłową reakcje poniżej:",
                       components=rows)


def setup(bot: PGBot):
    bot.add_cog(GroupMSG(bot))
