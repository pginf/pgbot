...
def get_user(username) -> List[QueryResult]:
    ...
    query_notify = QueryNotifyPrint(result=None,
                                    ...
                                    print_all=False,
                                    color=False)
    result = sherlock(username, site_data_all, query_notify, timeout=5)
    final_result: List[QueryResult] = []
    for r in result:
        if result[r].get("status").status == QueryStatus.CLAIMED:
            final_result.append(result[r].get("status"))
    return final_result
