...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def create_source(cls, ctx: SlashContext, search: str, *, loop: asyncio.BaseEventLoop = None):
        ...
        if data is None:
            raise Exception(f'Couldnt find anything that matches {search}')
        ...
        if process_info is None:
            raise Exception(f'Couldnt fint anything that matches {search}')
        ...
        if processed_info is None:
            raise Exception(f'Couldnt fetch {webpage_url}')
